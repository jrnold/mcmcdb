context("method mcmcdb_samples_chain_parameters")

local({
  source("data-McmcdbWide.R")
  
  test_that("mcmcdb_samples_chain_parameters test #1", {
    foo <- mcmcdb_samples_chain_parameters(test_wide2)
  ...
  test_that("mcmcdb_samples_chain_parameters test #2 (FUN)", {
    foo <- mcmcdb_samples_chain_parameters(test_wide2, FUN = length)
