context("method mcmcdb_samples_pararrays_chain")

local({
  source("data-McmcdbWide.R")
  
  test_that("mcmcdb_samples_pararrays_chain passes test #1", {
    foo <- mcmcdb_samples_pararrays_chain(test_wide2)
  ...
  test_that("mcmcdb_samples_pararrays passes test #2", {
    foo <- mcmcdb_samples_pararrays_chain(test_wide2, chain_id=1, pararrays="beta")
    expected <- structure(list(beta = structure(list(`1` = structure(c(1L, 7L, 13L, 2L, 8L, 14L, 3L, 9L, 15L), .Dim = c(3L, 3L))), .Names = "1")), .Names = "beta")
    expect_equal(foo, expected)
  })

  test_that("mcmcdb_samples_pararrays passes test #3 (FUN)", {
    foo <- mcmcdb_samples_pararrays_chain(test_wide2, FUN = length)
